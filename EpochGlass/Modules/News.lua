...
- New: EpochGlass now indents lines that wrap past the first line. Super helpful for visually distinguishing lines that belong to a single message. Note: If you don't like this new behavior, you can turn it off in the settings.
    ]]}
  },
  {
...
- New: EpochGlass now supports Prat's History module! As long as it's enabled, EpochGlass now restores your chat message history.

Bug fixes

- Fixed: There was an issue where messages that were received before EpochGlass initialized were lost (e.g. Guild Message of the Day). This has now been fixed.
    ]]}
  },
  {
...
- New: You no longer need a 16,000 DPI gaming mouse to move EpochGlass to the perfect spot. Now you can use sliders for window positioning.
...
- New: This thing! EpochGlass is getting constant updates with new features and fixes added almost weekly. We thought it would be a good idea to write up changes and new stuff between each release. Watch this space!
...
- Fixed: There was an issue with how EpochGlass saved the window position that was causing AceDB to throw a fit. This issue has been resolved.
    ]]}
  },
  {
...
- New: EpochGlass now supports Prat 3.0 URL links! This will now allow you click URL links in chat as long as you have Prat's UrlCopy module enabled.
...
- Fixed: Players have been experiencing issues with the edit box being visible even if it's not focused. This is caused by the chat style setting being set to "IM style". From now on, EpochGlass will automatically set the chat style to "Classic" so that you don't have to do it yourself.
    ]]}
  },
  {
...
- Fixed: There was a conflict with the ElvUI mover and EpochGlass. This has been fixed.
    ]]}
  },
  {
...
- New: EpochGlass now supports the "New tab" whisper mode! Other "temporary" chat windows are also now supported, including the Pet Battle tab.
- New: Major architecture changes for EpochGlass. You won't see any changes while using the addon, but rest well in knowing that we've given the engine a massive tune up.
    ]]}
  },
  {
...
- Fixed: Some players were experiencing issues when using EpochGlass with other addons. These issues have been addressed and should no longer happen.
    ]]}
  },
  {
...
- New: Players have been having a hard time figuring out how to move EpochGlass around. So we've added a new "Unlock Window" option when right-clicking the "General" tab. This is how the default chat UI unlocked its windows so hopefully this will be more obvious.
- New: EpochGlass now supports Prat Timestamps! If you're using Prat, you should find that timestamps are now displayed.
    ]]}
  },
  {
...
function News:OnEnable()
  ...
  frame:SetTitle("EpochGlass: Version history")
  frame:SetWidth(600)
  frame:SetHeight(400)
